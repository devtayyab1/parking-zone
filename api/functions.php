...
function get_client_ip() {
	...
	function GetAphDetails(){
		$ch = curl_init();
		$timeout = 5;
		curl_setopt($ch, CURLOPT_URL, 'https://parkingzone.co.uk/aphinfo');
...
$agent = 3;
$ip = get_client_ip();
$sql=$db->insert("INSERT INTO `fpp_tracking` (`id`, `ip`, `agent`, `date`, `ref_url`, `ref_src`,`url`) VALUES (NULL, '$ip', '$agent', '$date', '$ref_url','$ref_src','$url')");
}
function get_company_levy($company_id){
	...
	function send_sms($number, $ref, $is_hotel=false){
		//Z8wRnbuzOF96LSOpCqZxTwZLCU18JOkjMX100kqErqB28Q
		$url = "www.voodooSMS.com/vapi/server/sendSMS";
		$getString  ="?dest=".$number."&";
		$getString .="orig=travelez&";

		$getString .="msg=".urlencode("Thank you for choosing Travelez (Parking Zone). Your Airport Parking booking with Ref# ".$ref." has been confirmed, for arrival & departure procedure please check your email.")."&";
	...
	{
		global $db;
		$result = $db->get_row("SELECT createdate FROM ".$db->prefix."airports_bookings WHERE companyId = '".$id."' order by createdate  desc limit 1");
	...
	function facebooklogin($url=''){
		...
		if(!$fbUser){
			...
			if ($userData > 0) {
				...
				if($url > 0){redirect(SITE_ADDRESS.$url);}else{redirect(SITE_ADDRESS);};
				//$userData
			} else {
	...
	function get_post($ID = ''){
		global $db;
			...
			//Get post by id
			//echo "select * from " . $db->prefix . "pages where ".$this_query." and status = 'Yes'";exit;
    ...
    function getPromoDiscount($promoCode, $booking_price, $type='',$company_id='') {
		...
		if($promoDetails['discount_value']=='' || $_SESSION["por_src"]=='Active' || $_COOKIE['traffic']=='POR')
		{
           ...
           // return round($discount_amount,2);
		   return 1; //give 1 pound discount on wrong attempt
		}
	}
        ...
        if ($promoCode != '') {
            ...
            if ($promoDetails != "") {
                if($promoDetails['discount_campaign'] == 'General') {
                     $promofor = $db->get_row("SELECT * FROM " . $db->prefix . "discounts
                            WHERE promo = '".$promoCode."' AND status = 'Yes'
							$Discount_filter						
                            AND start_date <= '".date("Y-m-d")."' 
                            AND end_date >= '".date("Y-m-d")."'");
    ...
    function getModuleSettings() {
        global $db;
        $_settings = array();
        $settings = $db->select("SELECT * FROM " . $db->prefix . "modules_settings");
	...
	function check_extra($cid, $pid, $amount) {
		...
		$amount_total = $db->get_row("SELECT extra from " . $db->prefix . "companies_set_price_plans where id = '".$pid."' and cid = '".$cid."'");
	...
	function extra_amount($cid, $pid='') {
		global $db;
		$extra = $db->get_row("SELECT extra from " . $db->prefix . "companies_set_price_plans where id = '".$pid."' and cid = '".$cid."'");
		$extra = ($extra['extra']*1);
	...
	function APBookingPrice($cid, $aid, $no_of_days, $checkindate) {
		...
		if(is_numeric($cid)){
			...
			$companies = $db->select("SELECT fc.id as companyID,fapp.id, fasb.brand_name, fapb.after_30_days,IF( fapb.day_$total_days >0, fapb.day_" . $total_days . ", fc.parking_per_day_price * $total_days) as price FROM companies as fc
    ...
    function getBookingByRefferenceNumber($reffNo) {
		...
		//echo $reffParts[0];exit;
        switch ($reffParts[0]) {
            case 'TP':
                $bookingDetails['type'] = 'airport_parking';
                $bookingDetails['data'] = $db->get_row("SELECT * FROM ".$db->prefix."airports_bookings WHERE referenceNo = '".$reffNo."'");
				//echo "SELECT * FROM ".$db->prefix."airports_bookings WHERE referenceNo = '".$reffNo."'";
                break;
    ...
    function getBookingByTokenNumber($tokenNo) {
        global $db;
        $bookingDetails = false;
        $_data = $db->get_row("SELECT * FROM ".$db->prefix."airports_bookings WHERE token = '".$tokenNo."'");
		...
		$_data = $db->get_row("SELECT * FROM ".$db->prefix."transfer_booking WHERE token = '".$tokenNo."'");
        if (isset($_data['id']) && $_data['id'] > 0) {
           $bookingDetails['type'] = 'airport_transfer';
            $bookingDetails['data'] = $_data;
           return $bookingDetails;
        }
        return $bookingDetails;
	...
	function getBookingByID($id) {
		...
		b.total_amount,b.phone_number,b.cancelfee,b.payment_method,b.phone_number,b.deptFlight,b.returnFlight,b.deprTerminal,b.airportID, b.make, b.model,b.color, b.registration, c.arival, c.return_proc,c.terms
		from ".$db->prefix."airports_bookings as b 
		...
		from ".$db->prefix."airports_bookings as b join " . $db->prefix . "hotels as c on c.id = b.companyId where b.id='$id'");
        if (isset($_data['id']) && $_data['id'] > 0) {
            $bookingDetails['type'] = 'airport_hotel';
            $bookingDetails['data'] = $_data;
            return $bookingDetails;
        ...
        $_data = $db->get_row("select b.id as booking_id,b.email,b.companyId,b.referenceNo,b.departDate,b.returnDate,b.no_of_days,b.title,b.first_name,b.last_name,b.payment_status,
		b.total_amount,b.phone_number,b.cancelfee,b.payment_method,b.phone_number,b.deptFlight,b.returnFlight,b.deprTerminal,b.airportID, c.arival, c.return_proc,c.terms
		from ".$db->prefix."airports_bookings as b join " . $db->prefix . "hotel_parking as c on c.id = b.companyId where b.id='$id'");
        if (isset($_data['id']) && $_data['id'] > 0) {
            $bookingDetails['type'] = 'hotel_parking';
            $bookingDetails['data'] = $_data;
            return $bookingDetails;
    ...
    function newBookingNotification($token= '') {
        global $db;
        if ($token != '') {
            $_bookingDetails = $db->get_row("SELECT * FROM ".$db->prefix."airports_bookings WHERE id = '".$token."'");
	...
	function smtpmailers($to, $touser, $from, $from_name, $subject, $body,$files='',$mode='',$string_pdf='') 
	{ 
		...
		$mail->SMTPSecure = "tls";
		$mail->Port     = 587;  
		$mail->Username = 'pz@parkingzone.co.uk';
		$mail->Password = 'pzgsuite123#';
		  // $host = "ssl://smtp.gmail.com";                  
		$mail->Host     = "smtp.gmail.com";
		$mail->Mailer   = "smtp";
		$mail->AddReplyTo('no-reply@parkingzone.co.uk', 'Parkingzone');
		$mail->SetFrom('notifications@parkingzone.co.uk', 'Parkingzone');
		...
		$mail->AddAddress('bookings@parkingzone.co.uk', $touser);
		if($files != ""){
		$mail->AddAttachment($files);
		}
		if($string_pdf!='')
		{
		$mail->AddStringAttachment($string_pdf, "TAP_hotel_booking_voucher.pdf", $encoding = 'base64', $type = 'application/pdf');
		}
		if(!$mail->Send()) {
	...
	function email_signUP($customer_id,$password){
		...
		$from = "noreply@parkingzone.co.uk";
		$from_name = "Parking Zone";
    ...
    // More headers
    //$headers .= 'From: Fly Park Plus <noreply@flyparkplus.com>' . "\r\n";
	...
	b.payment_method,b.total_amount,b.cancelfee,b.booking_amount,b.phone_number,b.deptFlight,b.returnFlight,b.deprTerminal,b.returnTerminal,b.airportID, b.make, b.model,b.color, b.registration, IF(c.name IS NULL , b.booked_type, c.name) As cname, c.arival, c.return_proc,c.terms,c.admin_id,c.company_email,c.company_code
    from ".$db->prefix."airports_bookings as b 
	...
	$airport = $db->get_row("SELECT name FROM " . $db->prefix . "airports where id = '".$users['airportID']."'");
	$terminals = $db->get_row("select * from " . $db->prefix . "airports_terminals where id = '" . $users['deprTerminal'] . "'");
	$terminalz = $db->get_row("select * from " . $db->prefix . "airports_terminals where id = '" . $users['returnTerminal'] . "'");
	if($terminals ==""){$terminals = 'TBA';}else{$terminals = $terminals['terminal_name'];}
	if($terminalz ==""){$terminalz = 'TBA';}else{$terminalz = $terminalz['terminal_name'];}
	
	$explodes  = explode("PZ",$users['company_code']);
	$cname = $explodes[1];
	...
	if(empty($email)){$email = $users['email'];}
	
	if($email == 'admin'){
		$admin = $db->get_row("select * from users where id='1'");
		$email = $admin['email'];
		...
		$admin = $db->get_row("select * from users where id='".$users['admin_id']."'");
	...
	if(!empty($status)){
        ...
        $subject = $data['subject'] .(($status != 'Subscription')?(' Ref # '. $users['referenceNo']):'') ;
        ...
        $message = str_replace("[airport]",$airport['name'],$message);
		...
		$message = str_replace("[company]",$company_name,$message);
...
function send_quote($quote_link = '', $status, $email='') {
    global $db;
    if($_SERVER['HTTP_HOST'] == 'localhost'){ return false;}
    // Always set content-type when sending HTML email
    //$headers = "MIME-Version: 1.0" . "\r\n";
	...
	$status = isset($status) ? $status  : '';
        ...
        if($status == 'send_quote'){
            $sb_name = $db->get_row("select * from " . $db->prefix . "subscribers where email = '" . $email . "' limit 1");
            $user = $sb_name['name'];
        }
	...
	if(!empty($status)){
		...
		$message = str_replace("[username]",$user,$data['descriptions']);
		$message = str_replace("[quote_link]",$quote_link,$message);    
	}
    ...
    function getCompanyBookedSlots($_dropdate, $_dropoftime, $_pickdate, $_pickuptime, $_companyID)
    {
        ...
        $_bookings = $db->get_row("SELECT COUNT(id) as booked_slots FROM ".$db->prefix."airports_bookings as b WHERE companyId = '".$_companyID."' AND departDate BETWEEN '".$_from_date."' AND '".$_to_date."' AND removed = 'No' AND parent_id = 0 AND booking_status = 'Completed'");
        $_booked_slots = $_bookings['booked_slots'];
        $c_bookings = $db->get_row("SELECT COUNT(id) as booked_slots FROM ".$db->prefix."airports_bookings as b WHERE companyId = '".$_companyID."' AND departDate BETWEEN '".$_from_date."' AND '".$_to_date."' AND removed = 'No' AND parent_id > 0 AND booking_status = 'Cancelled'");
...
function notification_hotels($refNo,$status,$email,$voucher_file='')
{
	global $db;
	$from = "no-reply@parkingzone.co.uk";
	$from_name = "Parking Zone";
	$hotel_info = $db->get_row("SELECT * from " . $db->prefix . "hotel_bookings where referenceNo='".$refNo."' order by id asc limit 1");
	$hotel_name = ucwords(implode(' ',explode('_',$hotel_info["hotel_id"])));
	$data = get_email($status);
	$subject = $data['subject'] .' Booking# '.$hotel_info['order_id_api'];
	$username = $hotel_info["first_name"]." ".$hotel_info["last_name"];
	$message = str_replace("[username]",$username,$data['descriptions']);
	$message = str_replace("[email]",$hotel_info["email"],$message);
	$message = str_replace("[phone]",$hotel_info["phone_number"],$message);
	$message = str_replace("[order_no]",$hotel_info["order_id_api"],$message);   
	$message = str_replace("[ref]",$hotel_info["referenceNo"],$message);
	$message = str_replace("[api_error]",$hotel_info["reservation_error"],$message);
	$message = str_replace("[hotel]",$hotel_name,$message);
	$message = str_replace("[room]",$hotel_info["room_name"],$message);
	$message = str_replace("[checkin]",date('d/m/Y',strtotime($hotel_info["checkindate"])),$message);
	$message = str_replace("[checkout]",date('d/m/Y',strtotime($hotel_info["checkoutdate"])),$message);
	$message = str_replace("[nights]",$hotel_info["total_nights"],$message); 
	$message = str_replace("[adults]",$hotel_info["adults"],$message); 
	$message = str_replace("[children]",$hotel_info["children"],$message); 
	$message = str_replace("[price]",$hotel_info["total_amount"],$message);
	$message = str_replace("[refund_price]",$hotel_info["total_amount"],$message);
	
	if($_SERVER['HTTP_HOST'] != 'localhost'){
        //$sent_email = mail($email,$subject, $message,$headers); //$hotel_info['email']
        $sent_email = smtpmailers($email, $username, $from, $from_name, $subject, $message, $files,$mode,$voucher_file); //developmentfive1@gmail.com
    }
	if($sent_email =='1'){
		$msg = base64_encode('success:Email Succussfully send..!!');
	}else{
		$msg = base64_encode('error:Email Sending fail try again..!!');
	}
    $return_array = array('token' => $token, 'sent_email' => $sent_email , 'msg'=>$msg);
    return $sent_email;
}
function get_hotel_info($url,$json_data){	
	//global $db;
	$ch = curl_init(); 
	curl_setopt($ch, CURLOPT_URL, $url);    
	...
	curl_setopt($ch, CURLOPT_USERPWD, HOTEL_API_KEY.':'.HOTEL_API_PASSWORD);
	...
	$hotel = json_decode($result,true);
	return $hotel;
}
function get_voucher_info($url,$json_data,$type=''){	
	$url = $url."?data=".$json_data;
	$auth = base64_encode(HOTEL_API_KEY.":".HOTEL_API_PASSWORD);
	$context = stream_context_create([
		"http" => [
			"header" => "Authorization: Basic $auth"
		]
	]);
	$voucher_info = file_get_contents($url, false, $context );  
	if(strlen($voucher_info)>100)
	{
		if($type=='') { //show directly
	header("Content-Type: application/pdf");
	header("Content-length: ".strlen($voucher_info));
	echo $voucher_info;die($voucher_info);exit;
		}
		else return $voucher_info; //string data for email attachment
	}
	else
	{
	 return	"error";
