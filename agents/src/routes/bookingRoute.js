                                ...
                                try {
                                    db_query.query(
                                        ...
                                        function (error, results, fields) {
                                            ...
                                            sendEmailToConfrimBooking(orderID);
                                        }
                                    );
                                } catch (error) {
                                    db_query?.release();
...
const SaveAPHBOOKIng = formData => {
    return new Promise((resolve, reject) => {
        resolve(
            '{"BookingRef":"DC2CP","ArrivalDate":"10FEB23","DepartDate":"11FEB23","ArrivalTime":"0900","DepartTime":"0900","no_of_days":"1","CarParkCode":"LGW1","NumberOfPax":"1","ReturnFlight":"82345","DepTerminal":"TBA","OutFlight":"NA","RetTerminal":"TBA","CarReg":"VTC 123","CarMake":"NISSAN","CarModel":"ALTIMA","CarColour":"#000","Title":"MR","Initial":"A","Surname":"AMEEN","Telephone1":"923408906107"}'
        );
        //     var options = {
        //         method: 'POST',
...
bookingRoute.get('/', function (req, res, next) {
    ...
    if (booking_type == 'airport_parking') {
        sql = `SELECT * FROM airports_bookings as ab LEFT JOIN companies ON ab.companyId = companies.id    LEFT JOIN booking_transaction as bt ON ab.referenceNo = bt.referenceNo    WHERE email = '${email}' AND ab.last_name = '${last_name}' AND ab.referenceNo = '${booking_reference}'  LIMIT 1`;
    } else if (booking_type == 'hotel_booking') {
        sql = `SELECT * FROM tez_hotel_bookings WHERE email = '${email}' AND last_name = '${last_name}' AND referenceNo = '${booking_reference}'  LIMIT 1`;
    } else if (booking_type == 'lounge_booking') {
        sql = `SELECT  ab.*, bt.* , ab.referenceNo as booking_ref FROM lounges_bookings as ab LEFT JOIN lounges_transaction as bt ON ab.referenceNo = bt.referenceNo WHERE email = '${email}' AND ab.last_name = '${last_name}' AND ab.referenceNo = '${booking_reference}'  LIMIT 1`;
    } else if (booking_type == 'transfer_booking') {
        sql = `SELECT  ab.*, bt.* , ab.referenceNo as booking_ref FROM transfer_bookings as ab LEFT JOIN transfer_transaction as bt ON ab.referenceNo = bt.referenceNo WHERE email = '${email}' AND ab.last_name = '${last_name}' AND ab.referenceNo = '${booking_reference}'  LIMIT 1`;
    } else {
