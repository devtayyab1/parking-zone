            ...
            <div class="row">
                ...
                <div class="col-md-12"
                    ...
                    <div class="col-md-8">
                            ...
                            $countCompleted=0;
                              foreach($bookings_count as $booking)
                              {
                                ...
                                if($booking['booking_status']=='Completed') {
                                    ...
                                    if($booking->company == '363')
                                    {
                                    
                                      $company_share = (100 -  $share)/100 *  $booking->booking_amount ;
                                     $fly_share = ($share/100) * $booking->total_amount;
                                    ...
                                    else
                                    {
                                       ...
                                       $rev = $booking->booking_amount;
                                       $tot_rev += $rev;
                                    }
                                    $c_share += $company_share;
                                    $f_share += $fly_share;
                        ...
                        @endphp
                        <h2 class="badge badge-info" style="padding: 10px;"><i class="entypo-target"></i> No of
                        ...
                        <h2 class="badge badge-info" style="padding: 10px;"><i class="entypo-target"></i> Total
                            Booking Amount: <span id="total_share">{{$tot_rev}}</span></h2>
                            @if($adminName != '')
                        <h2 class="badge badge-info" style="padding: 10px;"><i class="entypo-target"></i> {{$adminName}}
                            share: <span id="total_share">{{$c_share}}</span></h2>
                            @else
                         <h2 class="badge badge-info" style="padding: 10px;"><i class="entypo-target"></i> Service Provider
                             share: <span id="total_share">{{$c_share}}</span></h2>
                            @endif
                        <h2 class="badge badge-info" style="padding: 10px;"><i class="entypo-target"></i> Booking Agent Share
                             <span id="net_share">{{$f_share}}</span></h2>
                        @endcan
                        @can('user_auth', ["Sources"])
                        @if($ORG != '0')
                    ...
                    <div class="col-md-4 text-right section-right" style="margin-top: 22px;">
                        ...
                        <a id="excel" class="btn btn-primary  btn-sm" href='{{ route("admin_booking_report_excel") }}?filter={{ Input::get("filter") }}&search={{ Input::get("search") }}&start_date={{ Input::get("start_date") }}&end_date={{ Input::get("end_date") }}&companies={{ Input::get("companies") }}&agentID={{ Input::get("agentID") }}&airport={{ Input::get("airport") }}&status={{ Input::get("status") }}&admins={{ Input::get("admins") }}&payment={{ Input::get("payment") }}&refund_via={{ Input::get("refund_via") }}&palenty_to={{ Input::get("palenty_to") }}&no_of_days={{ Input::get("no_of_days") }}&agentID={{ Input::get("agentID") }}&booking_source={{ Input::get("booking_source") }}'><i class="fa fa-file-excel-o"></i> Download Excel</a>
                        <a id="excel" target="_blank" class="btn btn-primary  btn-sm" href='{{ route("admin_booking_report_pdf") }}?filter={{ Input::get("filter") }}&search=&start_date={{ Input::get("start_date") }}&end_date={{ Input::get("end_date") }}&companies={{ Input::get("companies") }}&agentID={{ Input::get("agentID") }}&airport={{ Input::get("airport") }}&status={{ Input::get("status") }}&admins={{ Input::get("admins") }}&payment={{ Input::get("payment") }}&refund_via={{ Input::get("refund_via") }}&palenty_to={{ Input::get("palenty_to") }}'><i class="fa fa-file-pdf-o"></i>Download PDF</a>
                    </div>
                    @endcan
                </div>
				@endif
                                ...
                                <table id="simple-table" class="table  table-bordered table-hover">
                                    ...
                                    @foreach($bookings as $booking)
                                        ...
                                        <tr style="color:red;" id="expand_{{  $booking->id  }}">
                                            ...
                                            @php 
                                             $deptaure =   explode(" ", $booking->departDate);
                                             $date=date_create($deptaure[0]);
                                             $deptauredate =  date_format($date,"d/m/Y");
                                             $deptauretime = $deptaure[1];
                                             $return =   explode(" ", $booking->returnDate);
                                             $date= date_create($return[0]);
                                             $returndate =  date_format($date,"d/m/Y");
                                             $returntime =  $return[1];
                                            @endphp
                                            <td class="">@php echo $deptauredate . ' ' . $deptauretime @endphp</td>
                                            <td class="">@php echo $returndate . ' ' . $returntime @endphp</td>
                                            ...
                                            @can('user_auth', ["Sources"])
                                            <td class="">{{ $booking->traffic_src  }}</td>
                                            <?php
                    			    	       if(Str::contains($booking->ref_url, 'google')){
                    			    	           $ref = "Google";
                    			    	       }elseif(Str::contains($booking->ref_url, 'bing')){
                    			    	           $ref = "Bing";
                    			    	       }else{
                    			    	           $ref = $booking->ref_url;
                    			    	       } 
                                            ...
                                            @php
                                                ...
                                                $fly_share1 = ((($share/100)*$booking->booking_amount )-$booking->discount_amount)+$booking->booking_extra;
												 if($booking->companyId == '363')
												 {
												 $fly_share = ($share/100)*$booking->total_amount;
												 $fly_share = $fly_share  - $booking->discount_amount + $booking->booking_fee;
												 }
												 else
												 {
                                                $fly_share = ($share/100)*$booking->booking_amount;
                                                ...
                                                $sharePerc = 100-$share;
                                                $companyShare = ($sharePerc/100)*$booking->booking_amount;
                                            @endphp
											@can('user_auth', ["Amounts"])
											<td>£{{ round($fly_share,2) }}</td>
											<td>£{{ round($companyShare,2) }}</td>
...
<a id="cancel" class="btn btn-primary btn-xs" title="Cancel Booking Not show" onclick="return confirm('Are you sure want to cancel this booking..?')" href="{{ route('cancelNotShow',['id'=>$booking->id]) }}"><i class="entypo-cancel">Not Show</i></a>
                                                    ...
                                                    @can('user_auth', ["Booking History"])
                                                        <tr>
                                                            <td>History</td>
                                                            <td>
                                                                <i id="view_detail"
                                                                   onclick="getHistory('{{ $booking->id  }}')"
                                                                   class="btn btn-minier btn-success fa fa-eye"
                                                                   title="View"></i>
                                                            </td>
                                                        </tr>
...
//                        }
    ...
    function getHistory(id) {
		...
		data["id"] = id;
		data["_token"] = '{{ csrf_token() }}';
		$.post('{{ route("bookingdetail.showhistory") }}', data, function (data) {
			console.log("data===", data);
